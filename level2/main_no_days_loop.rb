...
class Tractus
  class << self
    ...
    def calculate_workdays(since_date, until_date, holidays)
      ...
      number_of_days - (number_of_days / 7 * 2) - extra_weekend_days - holidays
    end

    def holidays_in_period(since_date, until_date)
      BusinessTime::Config.holidays.uniq.reject{|holiday| holiday.sunday? || holiday.saturday? || holiday < since_date || holiday > until_date}.size
    end
...
Does not loop over the days, but uses the number of days in between to dates
...
time = Benchmark.measure {
  Tractus.calculate_days_available_in_period_for_developers(input_path, output_path)
}
puts time.real if show_time
# ruby mainfaster.rb  --input 'data.json' -o 'output.json' -t